...
### Multi-Node vLLM Inference (TP=16, Meta-Llama-3-8B)
...
**Key finding:** The TCPXO shim's `guest_config_checker` requires additional configuration for non-NCCL-test workloads. Multi-node vLLM was achieved by bypassing the shim (`NCCL_NET_PLUGIN`) and tuner (`NCCL_TUNER_PLUGIN`), using NCCL's built-in Socket transport for inter-node communication and NVLink P2P/CUMEM for intra-node.
...
for i in $(seq 1 8); do
  gcloud compute networks subnets create tcpxo-sub-$i \
    --network=tcpxo-net-$i --region=us-west1 --range="192.168.$i.0/24" --project=YOUR_PROJECT
done
...
# Delete the cluster
gcloud container clusters delete tcpxo-cluster --location=us-west1 --project=YOUR_PROJECT --quiet

# Delete VPC subnets, firewall rules, and networks
for i in $(seq 1 8); do
  gcloud compute networks subnets delete tcpxo-sub-$i --region=us-west1 --project=YOUR_PROJECT --quiet
  gcloud compute firewall-rules delete tcpxo-internal-$i --project=YOUR_PROJECT --quiet
  gcloud compute networks delete tcpxo-net-$i --project=YOUR_PROJECT --quiet
done
```
